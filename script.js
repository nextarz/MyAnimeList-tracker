...
const searchForm = document.getElementById('searchForm'); const queryInput = document.getElementById('queryInput'); const resultsContainer = document.getElementById('resultsContainer'); const airingContainer = document.getElementById('airingContainer'); const infoModal = document.getElementById('infoModal'); const modalTitle = document.getElementById('modalTitle'); const modalDesc = document.getElementById('modalDesc'); const modalInfoList = document.getElementById('modalInfoList'); const modalRating = document.getElementById('modalRating'); const modalTrailer = document.getElementById('modalTrailer'); const closeModalBtn = document.getElementById('closeModalBtn');

closeModalBtn.addEventListener('click', () => { infoModal.close(); modalTrailer.innerHTML = ''; searchForm.querySelector('button[type="submit"]').focus(); });

window.addEventListener('keydown', (e) => { if (e.key === 'Escape' && infoModal.open) { infoModal.close(); modalTrailer.innerHTML = ''; searchForm.querySelector('button[type="submit"]').focus(); } });

function formatDate(dateStr) { if (!dateStr) return "Unknown"; const d = new Date(dateStr); return isNaN(d) ? "Unknown" : d.toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }); }
...
const img = document.createElement('img'); img.src = anime.images.jpg.image_url; img.alt = Cover image for ${anime.title}; img.className = "w-20 h-28 rounded-md flex-shrink-0 object-cover border border-gray-600"; img.onerror = function () { this.src = 'https://storage.googleapis.com/workspace-0f70711f-8b4e-4d94-86f1-2a93ccde5887/image/647f8a56-45bb-416e-9ddd-10d7c0000b19.png'; };
...
const synopsis = document.createElement('p'); synopsis.className = 'line-clamp-4 mt-1 mb-2 text-slate-300 text-sm leading-snug'; synopsis.textContent = anime.synopsis || "No synopsis available.";
...
div.addEventListener('click', () => openModal(anime.mal_id)); div.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openModal(anime.mal_id); } });

return div; }

async function fetchAnime(query) { if (!query.trim()) { resultsContainer.innerHTML = <p class="text-center text-slate-400 mt-10">Please enter an anime name to search.</p>; return; }

resultsContainer.innerHTML = <p class="text-center text-slate-400 mt-10">Loading...</p>; try { const res = await fetch(${apiBase}/anime?q=${encodeURIComponent(query)}&limit=20); const { data } = await res.json();
...
} catch (err) { resultsContainer.innerHTML = <p class="text-center text-red-500 mt-10">Failed to fetch data. Please try again.</p>; console.error('Error in fetchAnime:', err); } }

async function fetchAllAiringAnime() { airingContainer.innerHTML = <p class="text-slate-400 text-center">Loading airing anime...</p>; let page = 1; let hasNext = true; const seen = new Set(); airingContainer.innerHTML = '';

try { while (hasNext) { const res = await fetch(${apiBase}/seasons/now?page=${page}); if (!res.ok) throw new Error(HTTP ${res.status}); const json = await res.json();
...
} catch (err) { airingContainer.innerHTML = <p class="text-red-500 text-center">Failed to load airing anime.</p>; console.error("Error fetching airing:", err); } }

async function openModal(id) { modalTitle.textContent = "Loading details..."; modalDesc.textContent = ""; modalInfoList.innerHTML = ""; modalRating.textContent = ""; modalTrailer.innerHTML = ""; infoModal.showModal();
...
} catch (err) { modalTitle.textContent = "Failed to load details"; modalDesc.textContent = "Unable to retrieve info. Please try again."; console.error('Error in openModal:', err); } }

searchForm.addEventListener('submit', e => { e.preventDefault(); fetchAnime(queryInput.value.trim()); });

window.onload = () => { queryInput.focus(); fetchAllAiringAnime(); };
